...
import YoyakuLogin from './YoyakuLogin'
import './YoyakuLogin.css'
  ...
  return (
    <header className="header" role="banner">
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: 12 }} aria-label="Site controls">
        ...
        {!yoyakuSession && (
          <button
            onClick={() => setShowYoyakuLogin(true)}
            title="Connect YOYAKU account"
            style={{
              background: 'rgba(255,255,255,0.06)',
              border: '1px solid rgba(255,255,255,0.1)',
              borderRadius: 8,
            ...
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(0,255,255,0.08)'; e.currentTarget.style.color = '#0ff' }}
      ...
      {showYoyakuLogin && (
        <>
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 299 }} onClick={() => setShowYoyakuLogin(false)} />
          <YoyakuLogin onClose={() => setShowYoyakuLogin(false)} />
        </>
      )}
